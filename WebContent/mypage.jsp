...
<!-- Header -->
		...
		<!-- Menu -->
			<nav id="menu">
				<ul class="links">
					<li><a href="Main_login">Home</a></li>
...
<%
   String url = "jdbc:oracle:thin:@localhost:1521:oraknu";
    ...
    query = "select p.name , p.ddate,p.maxnum , count(r.rid), c.name,r.rid from student s left outer join reserve r on(s.sid = r.sid) left outer join program p on(r.pid = p.pid) left outer join company c on(p.cid = c.cid) where s.id = '"+id+"' group by p.name,p.maxnum, p.ddate , c.name,r.rid";
    ...
    while (rs.next()) {   
    	...
    	if(pname!=null &&(!pname.equals(name)))continue;
		 ...
		 String query2 = "select count(*) from program p left outer join reserve r on(p.pid=r.pid) where p.name = '"+name+"'";   
